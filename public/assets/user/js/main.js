...
function viewport() {
    ...
    return (
        ...
        ((b = "client"), (a = document.documentElement || document.body)), { width: a[b + "Width"], height: a[b + "Height"] }
    );
}
// viewport size end
...
window.onload = function() {
    ...
    $(function() {
        if ($("#range-slider").length) {
            ...
            var target = $('#range-value');
            setTimeout(function() {
                var valueStart = $('#range-slider').data('from');
                var valueEnd = $('#range-slider').data('to');
                ...
                rangeSlider.noUiSlider.on('update', function (values) {
                    target.html( parseInt(values[0]).toLocaleString('vi-VN', {style: 'currency', currency: 'VND'}) + ' - ' + parseInt(values[1]).toLocaleString('vi-VN', {style: 'currency', currency: 'VND'}));
